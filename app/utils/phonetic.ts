...
export function phoneticDistance(phone1: string, phone2: string): number {
  ...
  for (const [cat1, cat2] of relatedPairs) {
    if ((groups1.includes(cat1) && groups2.includes(cat2)) ||
...
export function phoneticLevenshteinDistance(s1: string[], s2: string[]): number {
...
export function normalizeIPA(ipa: string): string[] {
  let normalized = ipa.replace(/[\/\[\]ˈˌːˑ.]/g, '').toLowerCase().trim();
  
  const phones: string[] = [];
  let i = 0;
  while (i < normalized.length) {
    const twoChar = normalized.substring(i, i + 2);
    if (['aɪ', 'aʊ', 'ɔɪ', 'eɪ', 'oʊ', 'tʃ', 'dʒ', 'ts', 'dz', 'ɛɪ'].includes(twoChar)) {
      phones.push(twoChar);
      i += 2;
    } else {
