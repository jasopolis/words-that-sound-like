  ...
  const performSearch = useCallback(
    (query: string) => {
      ...
      setTimeout(() => {
        ...
        const isIPAInput =
          /^[\/\[]/.test(query) || /[ɑæɔəɛɪʊʌɜθðʃʒŋɹ]/.test(query);
        ...
        if (!isIPAInput) {
          ...
          if (found) {
            ...
            setError(
              `Could not find pronunciation for "${query}". Try IPA notation.`,
            );
            setLoading(false);
            return;
          }
        ...
        const searchResults = dictionary
          ...
          .sort(
            (a: SearchResult, b: SearchResult) => b.similarity - a.similarity,
          )
          .slice(0, MAX_RESULTS);
  ...
  useEffect(() => {
    const loadDictionary = async () => {
      ...
      try {
        ...
        console.error(err);
        setError(
          "Unable to load pronunciation dictionary. Please check your connection.",
        );
      } finally {
        setLoading(false);
      }
    ...
    <div className="min-h-screen flex flex-col">
      <div className="w-full max-w-6xl mx-auto px-8 py-12 md:py-24 flex-1">
        ...
        <div className="mb-8">
          ...
          {status && <p className="text-gray-400 text-sm mb-2">{status}</p>}

          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-gray-400 text-sm">Try:</span>
            {(LANGUAGE_EXAMPLES[language] || LANGUAGE_EXAMPLES["en_US"]).map(
              (example) => (
                <button
                  key={example}
                  onClick={() => {
      ...
      <footer className="w-full flex gap-2 justify-center items-center text-center py-4 text-base text-white/80">
        <p>
          See on{" "}
          <a
            href="https://github.com/jasopolis/words-that-sound-like"
            target="_blank"
